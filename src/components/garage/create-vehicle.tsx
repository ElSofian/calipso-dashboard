		...
		modalIsOpen && (
			<div className="fixed inset-0 flex items-center justify-center z-1000 bg-black/20 backdrop-blur-sm" onClick={() => setModalIsOpen(false)}>
				<div className="relative flex flex-col p-6 bg-gradient-to-t from-gray-50 to-white rounded-xl shadow-sm dark:from-black dark:to-neutral-800 dark:border dark:border-neutral-700 dark:text-white" onClick={(e) => e.stopPropagation()}>
					<h2 className="text-lg font-semibold leading-none tracking-tight pb-1.5">Ajoutez un véhicule</h2>
					<p className="text-sm text-gray-500 pb-6">Ceci ajoutera un véhicule à la liste des véhicules de l&apos;entreprise.</p>
					...
					<form className="flex flex-col gap-4" onSubmit={handleSubmit}>
						<div className="flex flex-col gap-2">
							...
							<input className="text-sm font-medium rounded-sm border-1 border-gray-200 py-1 px-2 shadow-xs" type="number" name="vehicle_id" required />
						</div>
						<div className="flex flex-col gap-2">
							<label className="text-sm font-medium">Modèle</label>
							<input className="text-sm font-medium rounded-sm border-1 border-gray-200 py-1 px-2 shadow-xs" type="text" name="model" required />
						</div>
						<div className="flex flex-col gap-2">
							<label className="text-sm font-medium">Plaque</label>
							<input className="text-sm font-medium rounded-sm border-1 border-gray-200 py-1 px-2 shadow-xs" type="text" name="plate" required />
						</div>
